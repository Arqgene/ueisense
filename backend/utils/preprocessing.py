...
def extract_row_fuzzy_features(row):
    ...
    # hazy_vision covers the "wavy/distorted lines" question (visual_distortion in UI → hazy_vision payload key)
    hazy_vision = float(row.get("hazy_vision", row.get("glare_halos", 0)))
    ...
    episode_count = float(row.get("episode_count", 0))
    ...
    vis = fuzzy.visual_impairment_index(blurred, floaters, glare, peripheral, hazy_vision)
    ...
    rec = fuzzy.recurrence_index(similar_episodes, previous_uveitis, family_uveitis, episode_count)
